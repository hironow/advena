...
export const addUserAdmin = async (firebase_uid: string): Promise<void> => {
  ...
  await adminDb.runTransaction(async (tx) => {
    const utcNow = Timestamp.fromDate(new Date(new Date().toUTCString())) as unknown as string;

    const userRef = await tx.get(
      adminDb.collection(USER_COLLECTION).where("firebase_uid", "==", firebase_uid).limit(1),
    );
    if (userRef.empty) {
  ...
  if (useEmulator) {
    ...
    const eventBody = createCloudEventBody(USER_COLLECTION, newUserId, dataBase64);
    await sendCloudEvent(EVENTARC_ENDPOINT_ADD_USER, eventBody);
  }
};

export const getUserByFirebaseUidAdmin = async (firebase_uid: string): Promise<User | null> => {
  const adminDb = getAdminDb();
